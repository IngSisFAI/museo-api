...
function updateExcavacionBochones(req, res) {
    let excavacionId = req.params.excavacionId
    let update = req.body
    console.log('POST /api/excavacionBochon/:ExcavacionId UpdateExcavacion......')
    console.log(req.body)
...
function getExcavacionPorIdFoto(req, res) { // busca una excavacion por nombre
    let excavacion = req.params.fotoId
    Excavacion.findOne({ 'fotosExcavacion._id': excavacion }, (err, excavacion) => {
        if (err) return res.status(500).send({ message: `Error al realizar la petición: ${err}` })
        if (!excavacion) return res.status(404).send({ message: `La excavacion no existe buscada` })
        res.status(200).send({ excavacion: excavacion })
    })
