...
//-------------------
function getPersonasTermino(req, res) { // busca por nombres, apellidos o dni 
    let termino = req.params.termino
    let termino2 = Number.parseInt(termino)
    let expReg = new RegExp(termino)
    console.log(expReg)
    if (!termino2) termino2 = 0

    Persona.find({ $or: [{ 'nombres': expReg }, { 'apellidos': expReg }, { 'dni': termino2 }] }, (err, persona) => {
        if (err) return res.status(500).send({ message: `Error al realizar la petición: ${err}` })
        if (!persona) return res.status(404).send({ message: `La persona no existe buscada` })
        res.status(200).send({ personas: persona })
    })
